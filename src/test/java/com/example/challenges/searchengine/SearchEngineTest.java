...
class SearchEngineTest {
    ...
    record SearchEngineInputWrapper(List<SearchEngine.Listing> listings, int k) {}
    ...
    private final ChallengeTestRunner<SearchEngineInputWrapper, SearchEngine.Listing[]> runner = new ChallengeTestRunner<>(
            "search_engine",
            SearchEngineInputWrapper.class,
            SearchEngine.Listing[].class,
            input -> new SearchEngine()
                    .getTopListings(input.listings(), input.k())
                    .toArray(SearchEngine.Listing[]::new)
    );
